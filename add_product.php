...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // إذا لم توجد أخطاء، نقوم بإدخال البيانات في قاعدة البيانات
    if (empty($name_err) && empty($category_err) && empty($quantity_err) && empty($price_err)) {
        $sql = "INSERT INTO products (name, category, specifications, quantity, price, supplier, manufacturing_date) VALUES (?, ?, ?, ?, ?, ?, ?)";
        
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("sssidss", $param_name, $param_category, $param_specifications, $param_quantity, $param_price, $param_supplier, $param_manufacturing_date);
            ...
            if ($stmt->execute()) {
                ...
                echo "حدث خطأ أثناء إضافة المنتج. يرجى المحاولة مرة أخرى.";
            }
...
<head>
    ...
    <title>إضافة منتج جديد - نظام إدارة المصنع</title>
    <style>
        * {
            margin: 0;
            padding: 0;
